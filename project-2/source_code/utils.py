...
def show_annotated_clustering(X, clustering, labels, nf, ci):
    ...
    for i in range(n):
        plt.text(X[i, 0], X[i, 1], labels[i], fontsize=40, bbox=dict(facecolor=RGB_color(clustering[i], nb_clusters), alpha=0.5))

    plt.xlim(np.min(X[:, 0])-1, np.max(X[:, 0])+1)
    plt.ylim(np.min(X[:, 1])-1, np.max(X[:, 1])+1)
    plt.title(label='Closest to their cluster centre: {}'.format(ci), fontsize=60)

    #plt.show()
    figure = plt.gcf() # get current figure
    figure.set_size_inches(64, 36)
    plt.savefig("data/{}-clusters.png".format(nf), dpi = 50)
